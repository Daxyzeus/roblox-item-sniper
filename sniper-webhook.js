...
async function main() {
    try {
        ...
        webhookMessage(`logged into @${account.name} (${account.id})`)
        webhookMessage('getting item info')
        ...
        webhookMessage(`got item info for ${item.name}`)
        webhookMessage(`budget is set to ${budgetLimit} robux`)
        ...
        async function checkLoop() {
            ...
            try {
                ...
                if (lowestReseller.price <= budgetLimit) {
                    ...
                    webhookMessage('item has reseller within price range! attempting to buy...')
                    ...
                    if (bought.purchased) {
                        ...
                        await webhookMessage(`successfully bought item for ${fancyPrice}! exiting...`)
...
async function webhookMessage(msg) {
    try {
        const computer = os.userInfo().username + '@' + os.hostname()

        let payload = JSON.stringify({ content: msg,
            username: `schniper (${computer})` })
        ...
        console.error('failed to send webhook message, trying again soon:', err)
        setTimeout(() => webhookMessage(msg), 5000)
        return false;
    }
}
